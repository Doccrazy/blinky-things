...
#  define DEFAULT_PWM_PORT                                                                                                                 \
    0                       // On XIAO port 0 is for pins 1-5 port 1 is for all higher pins
...
AutoAnalog::AutoAnalog() {
  ...
  dacBitsPerSample = 8;
  autoAdjust       = true;
  for (int i = 0; i < MAX_BUFFER_SIZE; i++) { dacBuffer[i] = 0; }
...
#  endif
  ...
  if (enableADC == 3) { NRF_SAADC->SAMPLERATE = 16000000 / sampRate / 2 | SAADC_SAMPLERATE_MODE_Timers << SAADC_SAMPLERATE_MODE_Pos; }
}
...
void AutoAnalog::getADC(uint32_t samples) {
  if (enableADC == 2) {
    ...
    if (started) { NRF_I2S->TASKS_START = 1; }
  } else if (enableADC == 1) {
    ...
    if (adcBitsPerSample == 16) {
      if (!adcWhichBuf) {
        for (uint32_t i = 0; i < samples; i++) { adcBuffer16[i] = adcBuf0[i] << 2; }
      } else {
        for (uint32_t i = 0; i < samples; i++) { adcBuffer16[i] = adcBuf1[i] << 2; }
      }
    } else if (adcBitsPerSample == 8) {
      if (!adcWhichBuf) {
        for (uint32_t i = 0; i < samples; i++) { adcBuffer[i] = adcBuf0[i] >> 6; }
      } else {
        for (uint32_t i = 0; i < samples; i++) { adcBuffer[i] = adcBuf1[i] >> 6; }
      }
    }
    adcWhichBuf = !adcWhichBuf;
  }
...
void AutoAnalog::feedDAC(uint8_t dacChannel, uint32_t samples, bool startInterrupts) {
  if (enableDAC == 2) {
    ...
    if (dacBitsPerSample == 8) {
      if (whichBuf) {
        for (uint32_t i = 0; i < samples; i++) { dacBuf0[i] = dacBuffer[i] << 7; }
        NRF_I2S->TXD.PTR = (uint32_t) &dacBuf0[0];
      } else {
        for (uint32_t i = 0; i < samples; i++) { dacBuf1[i] = dacBuffer[i] << 7; }
    ...
    if (dacBitsPerSample > 8) {
      ...
      if (whichBuf) {
        for (uint32_t i = 0; i < samples; i++) { dacBuf0[i] = (uint16_t) (dacBuffer[i] << 1); }
      } else {
        for (uint32_t i = 0; i < samples; i++) { dacBuf1[i] = (uint16_t) (dacBuffer[i] << 1); }
      }
    }
    if (whichBuf) {
...
void AutoAnalog::adcSetup(void) {
  if (enableADC == 2) {
    ...
    // Configure pins
    NRF_I2S->PSEL.MCK = (I2S_PIN_MCK << I2S_PSEL_MCK_PIN_Pos) | (I2S_PSEL_MCK_CONNECT_Connected << I2S_PSEL_MCK_CONNECT_Pos) |
                        (I2S_PORT_MCK << I2S_PSEL_MCK_PORT_Pos);
    NRF_I2S->PSEL.SCK = (I2S_PIN_SCK << I2S_PSEL_SCK_PIN_Pos) | (I2S_PSEL_SCK_CONNECT_Connected << I2S_PSEL_SCK_CONNECT_Pos) |
                        (I2S_PORT_SCK << I2S_PSEL_SCK_PORT_Pos);
    NRF_I2S->PSEL.LRCK = (I2S_PIN_LRCK << I2S_PSEL_LRCK_PIN_Pos) | (I2S_PSEL_LRCK_CONNECT_Connected << I2S_PSEL_LRCK_CONNECT_Pos) |
                         (I2S_PORT_LRCK << I2S_PSEL_LRCK_PORT_Pos);
    NRF_I2S->PSEL.SDIN = (I2S_PIN_SDIN << I2S_PSEL_SDIN_PIN_Pos) | (I2S_PSEL_SDIN_CONNECT_Connected << I2S_PSEL_SDIN_CONNECT_Pos) |
                         (I2S_PORT_SDIN << I2S_PSEL_SDIN_PORT_Pos);
...
#  if defined __MBED__
    ...
    if (gain == -1) { gain = DEFAULT_PDM_GAIN; }
...
#  endif
    ...
    NRF_SAADC->CH[0].CONFIG =
      (SAADC_CH_CONFIG_RESP_VDD1_2 << SAADC_CH_CONFIG_RESP_Pos) | SAADC_CH_CONFIG_GAIN_Gain4 << SAADC_CH_CONFIG_GAIN_Pos |
...
void AutoAnalog::dacSetup(void) {
  if (enableDAC == 2) {
    ...
    NRF_I2S->PSEL.LRCK = (I2S_PIN_LRCK << I2S_PSEL_LRCK_PIN_Pos) | (I2S_PSEL_LRCK_CONNECT_Connected << I2S_PSEL_LRCK_CONNECT_Pos) |
                         (I2S_PORT_LRCK << I2S_PSEL_LRCK_PORT_Pos);
    NRF_I2S->PSEL.SDOUT = (I2S_PIN_SDOUT << I2S_PSEL_SDOUT_PIN_Pos) | (I2S_PSEL_SDOUT_CONNECT_Connected << I2S_PSEL_SDOUT_CONNECT_Pos) |
                          (I2S_PORT_SDOUT << I2S_PSEL_SDOUT_PORT_Pos);
    ...
    NRF_PWM0->PSEL.OUT[0] = (DEFAULT_PWM_PIN << PWM_PSEL_OUT_PIN_Pos) |
                            (PWM_PSEL_OUT_CONNECT_Connected << PWM_PSEL_OUT_CONNECT_Pos | DEFAULT_PWM_PORT << PWM_PSEL_OUT_PORT_Pos);
#  if defined DEFAULT_PWM_PIN2
    NRF_PWM0->PSEL.OUT[1] = (DEFAULT_PWM_PIN2 << PWM_PSEL_OUT_PIN_Pos) |
                            (PWM_PSEL_OUT_CONNECT_Connected << PWM_PSEL_OUT_CONNECT_Pos | DEFAULT_PWM_PORT2 << PWM_PSEL_OUT_PORT_Pos);
#  endif
